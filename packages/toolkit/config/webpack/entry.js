...
module.exports = ({
	...
	if (useBlockAssets) {
		...
		const blockMetadataFiles = blocksDirs.flatMap((dir) => {
			...
			// on Windows
			const files = glob(`${blocksSourceDirectory.replace(/\\/g, '/')}/**/block.json`, {
				absolute: true,
			});
			return files.map((file) => ({
				blockMetadataFile: file,
				blockDir: blocksSourceDirectory,
			}));
		});

		// add any additional entrypoints we find in block.json filed to the webpack config
		additionalEntrypoints = blockMetadataFiles.reduce(
			(accumulator, { blockMetadataFile, blockDir }) => {
					...
					assets
						...
						.forEach((rawFilepath) => {
							...
							const [entryFilepath] = glob(
								...
								`${blockDir.replace(/\\/g, '/')}/${entryName}.([jt]s?(x)|?(s)css)`,
								{
									absolute: true,
								},
							);
