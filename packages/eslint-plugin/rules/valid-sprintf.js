...
module.exports = {
	meta: {
		...
		messages: {
			...
			invalidFormatString: 'sprintf must be called with a valid format string',
			noPlaceholderArgs: 'sprintf must be called with placeholder value argument(s)',
			noPlaceholders: 'sprintf format string must contain at least one placeholder',
			placeholderMismatch:
	...
	create(context) {
		return {
			CallExpression(node) {
				...
				const functionName =
					callee.property && callee.property.name ? callee.property.name : callee.name;
				...
				switch (args[0].type) {
					...
					case 'CallExpression': {
						const argFunctionName = getTranslateFunctionName(args[0].callee);
				...
				for (const candidate of candidates) {
					const allMatches = candidate.match(REGEXP_SPRINTF_PLACEHOLDER);
					...
					const unorderedMatches = candidate.match(REGEXP_SPRINTF_PLACEHOLDER_UNORDERED);
					...
					if (!allMatches || (allMatches.length === 1 && allMatches[0] === '%%')) {
						context.report({
							node,
