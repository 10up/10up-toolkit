...
function makeFixerFunction(arg) {
	return (fixer) => {
		switch (arg.type) {
			...
			case 'Literal':
				return [fixer.replaceText(arg, replaceHyphenWithEnDash(arg.raw))];
...
module.exports = {
	meta: {
		...
		schema: [],
		messages: {
			foundHyphen: 'Use dashes (en or em) in place of hyphens for numeric ranges.',
		},
		fixable: 'code',
	},
	create(context) {
		return {
			CallExpression(node) {
				...
				for (const arg of candidates) {
					...
					if (!argumentString || !argumentString.match(HYPHEN_IN_RANGE)) {
