...
function getReferences(context, specifiers) {
	const variables = specifiers.reduce(
		(acc, specifier) => acc.concat(context.getDeclaredVariables(specifier)),
		[],
	);
	const references = variables.reduce((acc, variable) => acc.concat(variable.references), []);
	return references;
}
...
function collectAllNodesFromCallbackFunctions(context, node) {
	const functionSpecifiers = node.specifiers.filter(
		(specifier) =>
			specifier.imported &&
			['createRegistrySelector', 'useSelect', 'withSelect', 'withDispatch'].includes(
	...
	const functionArgumentVariables = functionReferences.reduce(
		(acc, { identifier: { parent } }) =>
			parent && parent.arguments && parent.arguments.length > 0
				? acc.concat(context.getDeclaredVariables(parent.arguments[0]))
				: acc,
		[],
	);
...
function collectAllNodesFromDirectFunctionCalls(context, node) {
	const specifiers = node.specifiers.filter(
		(specifier) =>
			specifier.imported &&
			['useDispatch', 'dispatch', 'useSelect', 'select', 'resolveSelect'].includes(
				specifier.imported.name,
			),
	);
...
function collectAllNodesFromObjectPropertyFunctionCalls(context, node) {
	const specifiers = node.specifiers.filter(
		(specifier) => specifier.imported && ['controls'].includes(specifier.imported.name),
	);
	const references = getReferences(context, specifiers);
	...
	const possibleCallExpressionNodes = referencesWithPropertyCalls
		.filter((reference) => reference.identifier.parent && reference.identifier.parent.parent)
...
function getFixes(fixer, context, callNode) {
	...
	const fixes = [fixer.replaceText(callNode.arguments[0], variableName)];

	const imports = context
		.getAncestors()[0]
		.body.filter((node) => node.type === 'ImportDeclaration');
	const packageImports = imports.filter((node) => node.source.value === importName);
	const packageImport = packageImports.length > 0 ? packageImports[0] : null;
	if (packageImport) {
		...
		if (!alreadyHasStore) {
			...
			fixes.push(fixer.insertTextAfter(lastSpecifier, `,store as ${variableName}`));
		}
	} else {
		const wpImports = imports.filter((node) => node.source.value.startsWith('@wordpress/'));
		const lastImport = wpImports.length > 0 ? arrayLast(wpImports) : arrayLast(imports);
	...
	create(context) {
		return {
			ImportDeclaration(node) {
				...
				const callbackFunctionNodes = collectAllNodesFromCallbackFunctions(context, node);
