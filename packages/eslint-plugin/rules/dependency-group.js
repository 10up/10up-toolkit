...
module.exports = {
	...
	create(context) {
		 ...
		 *
		 * @returns {string} Expected comment node value.
		 */
		 ...
		 *
		 * @returns {WPPackageLocality} Package locality.
		 */
		 ...
		 * @param {WPPackageLocality} locality Desired package locality.
		 *
		 * @returns {boolean} Whether comment node satisfies locality.
		 */
		function isLocalityDependencyBlock(node, locality) {
			...
			if (locality === 'External') {
				locality = '(External|Node)'; // eslint-disable-line no-param-reassign
			}
		 ...
		 *
		 * @returns {boolean} Whether node occurs before reference.
		 */
