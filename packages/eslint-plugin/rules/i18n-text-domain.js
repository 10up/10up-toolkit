 ...
 * @returns {undefined|*} Text domain argument.
 */
...
module.exports = {
	meta: {
		...
		messages: {
			...
			useAllowedValue: 'Use one of the allowed text domains: {{ textDomains }}',
		},
		fixable: 'code',
	},
	create(context) {
		...
		const allowDefault =
			allowedTextDomains.length === 0 || allowedTextDomains.includes('default');
			...
			CallExpression(node) {
				...
				if (textDomain === undefined) {
					if (!allowDefault) {
						const addMissingTextDomain = (fixer) => {
							...
							return fixer.insertTextAfter(lastArg, `, '${allowedTextDomains[0]}'`);
						};
				...
				if (value === 'default' && allowDefault) {
					const removeDefaultTextDomain = (fixer) => {
						...
						return fixer.removeRange([previousArg.range[1], range[1]]);
					};
				...
				if (allowedTextDomains.length && !allowedTextDomains.includes(value)) {
